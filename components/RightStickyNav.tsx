...
import { GREY_2, GREY_5, GREY_6, HEADLINE_TEXT, ORANGE_PRIMARY } from './colors'
...
export function RightStickyNav({ headers }){
        ...
        <Group>
            <Heading>ON THIS PAGE</Heading>
            {
                headers.map(item => (
                    <Item 
                        key = {item.key}
                        className = {`tab-${item.level} ${activeItem === item.key ? 'active' : ''}`} 
