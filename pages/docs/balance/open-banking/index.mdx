...
Dwolla’s Exchange Sessions streamlines the Instant Account Verification (IAV) process for your application. When initiating an Exchange Session, Dwolla securely establishes a connection with your chosen Open Banking Provider (OBP). The user is then redirected and presented with a screen to select their financial institution and authenticate using their online baking credentials. The specific authentication flow may vary slightly depending on the chosen OBP, but Exchange Sessions handles the communication and data exchange throughout the process, ensuring a smooth and secure user experience.
