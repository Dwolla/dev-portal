...
To maintain a smooth user experience, your application needs to handle these scenarios gracefully. Dwolla's API provides an `UpdateCredentials` error response to signal when a user's bank connection needs to be refreshed. Additionally, Dwolla will also send a `customer_exchange_reauth_required` webhook denoting whether an exchange is pending deactivation, or a `customer_exchange_deactivated` webhook denoting that an exchange has been deactivated and requires re-authentication.
