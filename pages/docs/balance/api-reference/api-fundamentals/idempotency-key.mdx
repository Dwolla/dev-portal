...
meta:
  ...
  description: "Learn how to use the Idempotency-Key header to prevent duplicate operations and manage resource creation in the Dwolla API. Find out how to ensure idempotency with unique keys and handle response codes effectively."
---
...
For example, if a request to [initiate a transfer](/docs/balance/api-reference/transfers/initiate) fails due to a network connection issue, you can reattempt the request with the same idempotency key to guarantee that only a single transfer is created.
