...
weight: 2
meta:
  title: Dwolla Connect Webhook Events | Dwolla API Documentation
...
| topic      | Type of action that occurred within Dwolla. <br/> **Format:** `<resource>.<related_resource>:<type/action>` or `<resource>:<type/action>`. <br/> For example, the event topic for when a funding source belonging to an external party is removed is `external_party.funding_source:created`.                                                                                                                                                                                                    |
