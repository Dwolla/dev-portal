...
This section contains information on how to create an exchange session for an <span><ContentTooltip preset="external-party">external party</ContentTooltip></span>. The creation of an exchange session initiates the Instant Account Verification process with a trusted open banking partner. The creation of an exchange session requires a `_link` that identifies the open banking ecosystem partner, which can be found by calling the [list exchange partners](http://localhost:3000/docs/connect/api-reference/exchanges/list-exchange-partners) endpoint.
...
| ----------- | --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 201         | Created         | The Dwolla API accepted the request and created an exchange resource. You can reference the Location header to retrieve a link that represents the created exchange session resource. |
