...
- Use the [create an exchange session](/api-reference/open-banking/create-exchange-session-for-an-external-party) endpoint of the Dwolla API to initiate an Exchange Session for an External Party.
...
- Make an API call to [retrieve an exchange session](/api-reference/open-banking/retrieve-an-exchange-session). Upon success, Dwolla will return a `external-provider-session` URL where your application will redirect the user to a secure login.
...
- Upon successful authorization, you'll obtain specific user and selected account details from the open banking provider which will be used to [create an exchange](/api-reference/exchanges/create-exchange-for-an-external-party).
...
- Using the [create a funding source endpoint](/api-reference/funding-sources/create-for-external-party), you'll create a funding source by passing in the exchange resource from the previous step.
- Dwolla returns a 201 response and triggers a `external_party.funding_source:created` webhook indicating the funding source and account verification is complete.
