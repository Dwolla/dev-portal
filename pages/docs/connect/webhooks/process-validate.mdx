...
guide:
  step: 2
meta:
  title: Process and Validating Dwolla Webhooks | Dwolla Connect | Dwolla API Documentation
  description: Processing and validating webhooks for payments within your application by utilizing the Dwolla Connect API.
---
...
Although Dwolla tries to only send a webhook once, it is possible that the same webhook may be sent more than once. Because of this, it is recommended to check an internal database against the webhook’s [Event](https://developers.dwolla.com/api-reference/connect/events) ID to ensure that the same business logic is not processed multiple times.
