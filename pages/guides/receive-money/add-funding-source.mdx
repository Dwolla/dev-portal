...
If you have an active webhook subscription, you will receive both the `customer_funding_source_added` and `customer_funding_source_verified` webhook immediately following the request to Dwolla to add a funding source using a Plaid `processorToken`.
