...
For isolated functions or non-flow components, a single [client token](https://developers.dwolla.com/api-reference/drop-in-components/create-client-token) can be utilized. With `token` you can supply a function that we can call that returns a promise containing a token.
...
This endpoint should simply act as a passthrough to the [client-tokens endpoint](https://developers.dwolla.com/api-reference/drop-in-components/create-client-token). It takes in the request body and makes a client-token call. Upon success, it returns JSON-encoded string token in the response.
