...
Regardless of which drop-in component is being used by your application, the dwolla-web.js JavaScript library will require a unique “client-token” or a server side route that is used to generate a client token to be passed in on [configuration](/using-drop-in-components#configure-dwolla-webjs). A client token contains granular permissions and is scoped to the end-user/Customer that is performing the action within your web application.
