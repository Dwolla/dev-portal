...
For isolated functions or non-flow components, a single [client token](https://docs.dwolla.com/#create-a-client-token) can be utilized. With `token` you can supply a function that we can call that returns a promise containing a token.
