...
In addition to using the Dwolla Components Library, you’ll need a [server-side SDK](/sdks-tools) in order to facilitate the API request to Dwolla to retrieve a [client-token](https://developers.dwolla.com/api-reference/#create-a-client-token). A client-token is a unique token scoped to the end-user/Customer performing the action on the front-end of your web application.
