...
There are a number of banks that aren’t found in the bank search. These banks are generally smaller financial institutions. When a bank isn’t returned in the bank search, we’ll prompt the user to input the account and routing number of their bank. Once they click agree and continue, we’ll attempt to look up a bank with the provided information. If a bank is found, we’ll add it as an `unverified` funding-source to the Customer and return a screen for the user to authenticate using their online banking credentials. Once the online banking credentials are submitted, we receive back information that allows us to verify the bank account. Alternatively, if the user fails to correctly enter their online credentials, the funding-source will fallback to microdeposits verification after two failed attempts.
...
- Verify the funding-source using [Microdeposits](https://developers.dwolla.com/guides/micro-deposit-verification#verify-a-bank-account-with-micro-deposits)
