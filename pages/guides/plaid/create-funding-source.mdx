...
This step assumes you’ve [created a Customer](https://developers.dwolla.com/api-reference/#create-a-customer) for the user that you’ve authenticated or will authenticate with Plaid. Using the `processor_token` obtained from the previous step, you’ll pass this in as the value of the `plaidToken` request parameter; along with a funding source `name` in the request to [create a funding source for a Customer](https://developers.dwolla.com/api-reference/#new-funding-source-for-a-customer).
