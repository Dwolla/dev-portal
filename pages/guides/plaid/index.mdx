...
Your Customers will authenticate information with their financial institution through [Plaid Auth](https://plaid.com/products/auth/), and select the bank or credit union account they wish to use for initiating bank transfers. Behind the scenes, you’ll receive a unique token from Plaid that will be used to [create a funding source for a Customer](https://docs.dwolla.com/#create-a-funding-source-for-a-customer) via the Dwolla API. The Dwolla and Plaid partnership offers an elegantly designed and secure way for our joint Customers to verify account ownership with access tokens, removing sensitive financial information from the transaction stream.
