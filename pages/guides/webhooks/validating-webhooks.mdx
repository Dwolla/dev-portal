...
The `topic` field of an event holds [a description](https://developers.dwolla.com/api-reference/events) of the event, which is similar the subject of an e-mail message. The `webhook` itself contains `_links` to the resource impacted by the event that can be used to retrieve more information about the webhook you have received.
...
<AlertBar variation="info">
  As a best practice, we recommend validating webhooks by generating the same
  SHA256 HMAC hash and comparing it to the signature sent with the payload.
  <br />
  <strong>Note:</strong> We do not recommend nor support relying on IP
  whitelisting as a method of validating webhooks. Dwolla's IPs are dynamically
  allocated with no defined range and are subject to change.
</AlertBar>
