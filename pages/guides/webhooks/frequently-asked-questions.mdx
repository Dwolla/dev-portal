...
>
  <p>
    ...
    , filtering all that have zero attempts (in other words, the `attempts`
    array is empty), and{" "}
    <a href="/api-reference/webhooks/retry">
...
>
  <p>
    ...
    , an embedded <code>attempts</code> array will include a{" "}
    <code>response</code> object that includes a <code>statusCode</code>{" "}
    property indicating the HTTP status code that Dwolla received when the
    webhook attempt was made. If the status code that Dwolla received was >=300,
    then the attempt was considered to have failed.
  </p>
</Collapsible>
