...
<AlertBar variation="warning">
  ...
  `retry` status; simply passing the same information will result in the same
  insufficient scores. All fields that were required in the initial Customer
  creation attempt will be required in the retry attempt, along with the full
  9-digit SSN.
</AlertBar>
<br />
...
> [Learn more](/concepts/sandbox) on testing KBA in the sandbox environment.
