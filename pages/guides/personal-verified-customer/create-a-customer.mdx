...
To create a personal verified Customer, use the [create a Customer](https://developers.dwolla.com/api-reference/#create-a-customer) endpoint. A personal verified Customer is determined by setting the value of the `type` request parameter to `personal` and including additional fields required for identifying the individual.
