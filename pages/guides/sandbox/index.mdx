...
<AlertBar variation="warning">
  Real bank account numbers should not be used in a testing environment like Sandbox.
</AlertBar>

### Test Virtual Account Numbers (VANs) Transfer

External transfers can be simulated in the Sandbox by using the sandbox-simulations endpoint with a non-blank body and a type field set to `virtual`. Up to 10 transfers at a time can be included in one call to the sandbox simulations endpoint. Transfers will be created in a `pending` or `failed` status depending on whether the VAN has been removed or the Customer has been `deactivated` or `suspended`. Pending transfers will be processed automatically during the following times (all in Central Time), 
...
<AlertBar variation="info">
The times listed above only apply to the Sandbox testing environment. In Production, external transfers are processed at 11 AM, 1:15 PM and 5:45 PM (Central Time).
</AlertBar>
...
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| R01         | Insufficient Funds: This value is used to simulate funds failing from the source bank account (debit).                                                                                                                                                                               |
| R03         | No Account/Unable to Locate Account: This value is primarily used to simulate funds failing to the destination bank account (credit). The funding source will be automatically removed from Dwolla when this return code is triggered.                                               |
...
### Test VANs transfer failures

Similar to [testing ACH bank transfer failures](#test-bank-transfer-failures), you can test VANs transfer failures by specifying an "R" code in the "name" parameter when creating a VAN (e.g. “R08 Failure Test VAN”). The return code must be at the beginning of the VAN funding source’s name. **Note** that it is not case sensitive. 
...
| R02  | No Account/Unable to Locate Account: This value is primarily used to simulate funds failing to the destination bank account (ACH credit). The funding source will be automatically removed from Dwolla when this return code is triggered. | During processing of pending transactions.                                          |
