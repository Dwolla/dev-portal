...
- Added support for creating [access-tokens](https://developers.dwolla.com/api-reference/#application-authorization) and [client-tokens](https://developers.dwolla.com/api-reference/#create-a-client-token) in the dwolla-swagger-php SDK. Check it out on [Github](https://github.com/Dwolla/dwolla-swagger-php/releases/tag/1.6.0).
...
- Removed support for [uploading personal identification documents](https://developers.dwolla.com/api-reference/#create-a-document-for-a-customer) in the file format of `.pdf`. A validation error will be returned with a `code` of `"Invalid"` and a `message` of `"Invalid file type"`.
...
- Head over to our [API Docs](https://developers.dwolla.com/api-reference/#knowledge-based-authentication-kba) or check out our [blog post](https://www.dwolla.com/updates/kba-streamline-user-onboarding-individuals/) to learn more.
...
- Head over to our [API Docs](https://developers.dwolla.com/api-reference/#retrieve-a-funding-source-balance) or check out our [resource article](https://developers.dwolla.com/resources/balance-funding-source.html#total-and-available-balance) to learn more.
...
- Reference our [API Reference Docs](https://developers.dwolla.com/api-reference/#application-authorization) to learn more.
...
- Added the new [benefical owners endpoints](https://developers.dwolla.com/api-reference/#beneficial-owners) and [webhooks](https://developers.dwolla.com/api-reference/#event-topics---a-hrefbeneficial-ownersbeneficial-ownersa).
...
- Release support for a new `clearing` request parameter when [initiating a transfer](https://developers.dwolla.com/api-reference/#initiate-a-transfer). Clearing is a JSON object that supports specifying same-day and standard ACH clearing per API request. Note: The clearing request parameter is a premium feature available for Dwolla API customers.

### 2016-12-01

**DEPRECATED**

- Remove the `scope` attribute from the [application access token](https://developers.dwolla.com/api-reference/#application-authorization) response.
...
- Change the `phone` request parameter from required to optional when [creating a Customer](https://developers.dwolla.com/api-reference/#create-a-customer).
...
Remove the `profileId` request parameter from the [Off-Site Gateway](https://developers.dwolla.com/api-reference/#checkouts) in API v1.
...
- Change in functionality for [removing a funding source](https://developers.dwolla.com/api-reference/#remove-a-funding-source) in API v2. The method for removing a funding source changes from a `DELETE` to a `POST` with the need to supply `{"removed": true}` in the body of the request.
- A `removed` attribute is added to the [funding source object](https://developers.dwolla.com/api-reference/#funding-source-resource).
- A `removed` querystring request parameter is supplied when listing an [Account](https://developers.dwolla.com/api-reference/#list-funding-sources-for-an-account) or [Customer’s](https://developers.dwolla.com/api-reference/#list-funding-sources-for-a-customer) funding sources. By default, all funding sources are returned from the listing unless the `removed` request parameter is set to `false`.
