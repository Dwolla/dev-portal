...
- Added support for including [facilitator fee](/concepts/facilitator-fee) when creating transfers from Verified Customer's Bank into their Dwolla balance.

### 2022-10-27

**ADDED**

- Added the `correlationId` attribute to the [webhook payload](/concepts/webhook-events#webhook-payload) for transfer related webhooks if a value was specified on transfer creation. Previously, fees could only be applied to transactions between two parties.
...
- Added support for simulating `document` status for Business Verified Customers in Sandbox. Check out our [Testing in the Sandbox](/guides/sandbox#testing-customers) guide to learn more.
...
- Release support for a new (optional) `backButton` and `subscriber` options for IAV within [dwolla.js](/guides/dwolla-js/instant-account-verification/configure-and-call-function#options-and-customization). Note: Dwolla.js is a premium feature only available for Dwolla [API](https://www.dwolla.com/platform) customers.
