...
Note that for `VANs`, additional details about the external transaction will be available in a achDetails JSON object in the transfer `GET` response. Either the source or the destination will be present in the object depending on the direction of money movement.
