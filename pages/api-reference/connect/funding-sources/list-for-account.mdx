...
| removed   | no       | string | Filter removed funding sources. Set to `false` to filter out removed funding sources from list (i.e. - `/accounts/{id}/funding-sources?removed=false`) or `true` to include only removed funding sources. |
