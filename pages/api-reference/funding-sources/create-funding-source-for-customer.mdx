...
There are two methods available for adding a bank or credit union account to a Customer. You can either collect the Customer's bank account information and pass it to Dwolla via the [new Customer funding source](#create-a-funding-source-for-a-customer) endpoint, or you can send the Customer through the the [Instant Account Verification](https://developers.dwolla.com/guides/dwolla-js/instant-account-verification) (IAV) flow which will add and verify a bank account within seconds.
