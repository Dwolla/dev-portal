...
When a bank funding source with an `unverified` status is removed, an attempt to re-add it within 48 hours of the initial created date will re-activate the removed funding source and maintain the same `id`. Removing and readding a `VAN` however, will create a different `id`.
