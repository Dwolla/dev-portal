...
All requests should supply the `Accept: application/vnd.dwolla.v1.hal+json` header. `POST` requests can either specify the `Content-Type: application/vnd.dwolla.v1.hal+json` or `Content-Type: application/json` header. Request and response bodies are JSON encoded.
