...
Additional details about an external transaction will be available in the when [retrieving the transfer](https://developers.dwolla.com/api-reference/transfers/retrieve) from the API. Either the source or the destination will be present in the object depending on the direction of the funds transfer.
