...
Since a fee is a separate transfer in itself, it will show up in the transfer listing of either the [Account](https://developers.dwolla.com/api-reference/#list-and-search-transfers-for-an-account) or [Customer](https://developers.dwolla.com/api-reference/#list-and-search-transfers-for-a-customer) resource, depending on which party is sending or receiving the fee. To correlate a fee to the transfer that the fee was charged on, a key of `created-from-transfer` will be returned in the list of links on a unique transfer resource. The `created-from-transfer` key can also be used to differentiate a fee from other transfer types.
