...
Before you can initiate a debit or credit transfer using same-day clearing, you must first have a [Customer created](https://docs.dwolla.com/#create-a-customer) and a [funding source](https://docs.dwolla.com/#create-a-funding-source-for-a-customer) attached for the user. If it’s a debit transfer, then the funding source must be `verified` before you can pull funds. For a credit transfer, the funding source can be `unverified` or `verified`.
