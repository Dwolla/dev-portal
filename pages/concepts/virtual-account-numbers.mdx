...
- Currentl, users that are enabled for VANs will not have access to RTP, wire or Push-to-debit features.
...
The Dwolla API supports methods for [creating a VAN for a Verified Customer](/api-reference/funding-sources/create-funding-source-for-customer#van-virtual-account-number) and [creating a VAN for a Master Account](/api-reference/accounts/create-a-funding-source#van-virtual-account-number), as well as [retrieving routing details](/api-reference/funding-sources/retrieve-a-van-account-and-routing-number) and [removing a virtual account](/api-reference/funding-sources/remove).
...
| Name                      | Description                                                                                                                                                                  |
| ------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| companyEntryDescription   | Describes the purpose of the transaction. Valid values include: `REVERSAL`, `RECLAIM`, `NO CHECK`, `AUTOENROLL`, `REDEPCHECK`, `RETURN FEE`, `RETRY PMNT`, and `HEALTHCARE`. |
| routingNumber             | Routing number of Original Depository Financial Institution (ODFI)                                                                                                           |
