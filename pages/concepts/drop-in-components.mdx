...
The `dwolla-document-upload` component can be used if a Verified Customer or Beneficial Owner has a `document` status and needs to upload an identifying document to complete verification of their account. A government-issued document might be required to verify an individual or business' identity.
