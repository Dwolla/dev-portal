...
- Updated `dwolla.configure` `token` callback function to receive `_links`, `action`, and `links`, rather than just `action` and `links`. This will allow immediate pass-through without modification to the Dwolla API while maintaining backwards compatibility for existing implementations.
